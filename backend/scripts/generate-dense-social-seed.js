 ...
 * 120 users with realistic avatars, bios/statuses, Unsplash posts, stories, friends, likes.
 *
...
const lines = [];
lines.push('-- Dense social seed: 120 users, posts, stories, friends, engagement');
...
const engRows = [];
let eng = 0;
for (let p = 1; p <= Math.min(POST_COUNT, 200); p++) {
  for (let u = 1; u <= 4; u++) {
    eng++;
